...
Read_mul(false, n) = sink_r_0 ? x0 -> Read_mul(true, x0)
                  [] sink_r_1 ? x1 -> Read_mul(true, x1)
                  [] sink_r_2 ? x2 -> Read_mul(true, x2)
                  [] sink_r_3 ? x3 -> Read_mul(true, x3)
                  [] sink_r_4 ? x4 -> Read_mul(true, x4)

Read_mul(true, x) = sink_r_0 ? y0 -> STOP
                 [] sink_r_1 ? y1 -> STOP
                 [] sink_r_2 ? y2 -> STOP
                 [] sink_r_3 ? y3 -> STOP
                 [] sink_r_4 ? y4 -> STOP
...
\begin{frame}{Unclocked vs. clocked}
     % TODO Sammenlign med to grafer paa en
\end{frame}
