...
One way of initialising the network is by instantiating the processes with constants or hard code internal values, which then acts as the starting point of the network. Another way of initialising data in the SMEIL network, is to have a seperate process creating data for the network. We call this a data generator process. In Listing \ref{lst:addone_data_generation_example} we see a SMEIL network with two processes. The \texttt{Add} process receives an input value, increments 1 to it and writes it out on the outbus bus. The \texttt{Id} process receives an input and writes it to its output bus imediately.
-------------------
24-10-18
------------------
% ...Write something more here.. I am just too tired to do that now.
% TODO: Here I can introduce the addone problem as well as some other problem where there are a data generator process.

%%% Generator processes - clock cycles
TAPS must be able to handle all these different ways of generating data in different process structures and must ensure that the data used in the generated program are representing all possible values from the SMEIL program.
