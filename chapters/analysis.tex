...
\end{minted}
% TODO: This example is the same as listing lst:smeil_input_parameter in implementation
...
\paragraph{Variables}
Variables are very similar to constants but simply hold mutable values within the process. In SMEIL variables persist their values in between clock cycles, which mean that it is possible for a process to save a value or results and reuse it in the next clock cycle.
...
% TODO: Write something about how instances can be defined with an instance name or without and how that also impacts how the channels are defined as parameters for the processes and how one process can be instantiated several times but must have a instance name, otherwise it will fuck up. I think truls writes that only one instance can be wildcard in one network.
...
It it also possible to give the output bus as a process parameter. An example of this can be seen below where the process \texttt{addone} reads from its \texttt{input} bus and writes to its \texttt{output} bus, both defined as parameters.
