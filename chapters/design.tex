...
As mentioned in the Analysis chapter we synchronise processes together to create a network in \cspm{} similar to the network defined in SMEIL.
...
Since we can only synchronise two processes at a time, the network quickly become very complex, but this is not a problem since we are auto generating it. It is, however, a challenge to ensure that all processes connect with the other processes on the correct channels.

% NOTE: here I start talking about how it was when I wrote the paper. Maybe I should just accept that some of the stuff in the paper is not relevant anymore or I realised that it could be changed for the better. That is okay
In the first version of TAPs, the network read from the input bus so the process itself receives the input value as parameter. The reason for this design decision was that we wished to avoid using a..

% NOTE: Here I stopped. I realised that I need to figure out how to structure this and what I actually should write, before I continye. Because it is a waste of time to write stuff that I am not going to use anyway..
...
% ---------------------------
% TODO: Show the grammar and how it can be difficult to see if the expression is a input communication or if it is a arithmetic statement.
The input bus from the SMEIL process is transformed to a variable containing a value instead but this does not change the structure of the arithmetic statements.
...
% %%%% Monitor processes %%%%%
When creating the assertions, we decided to create separate assert functions to keep the code structure clean. We know that for each \cspm{} channel there must be an assertion, except for the input channel. There might be a case where we are not interested in verifying all channels, but we add asserions
