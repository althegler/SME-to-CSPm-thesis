...
The work was initially presented in the paper \textit{BPU Simulator}~\cite{Rehr2013}, which introduced a high abstraction level simulation. Therefore the subject was explored more in detail in the master's thesis project \textit{Generation of FPGA Hardware
...
The SME model supports both synchronous and asynchronous processes. A synchronous process is run during every clock cycle and an asynchronous process are only run when receiving a signal on the input bus. Unfortunately, SMEIL does not currently support asynchronous processes. %TODO: Ask truls if it is implemented in other sme implementations.
\begin{listing}
