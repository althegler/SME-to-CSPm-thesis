...
\end{minted}
\caption{Sections of the translated \texttt{addone} network. The \texttt{Add} process has restrictions included to ensure no values above 10. The monitor process defines this range along with the acceptance of the dummy value -1. This example has been manually translated due to limitations of the clocked version of TAPS. See full example in Listing~\ref{lst:cspm_addone_full} in the appendix.}
...
The full code for the clocked seven segments display example can be seen in Listing \ref{lst:cspm_clocked} in Appendix.
